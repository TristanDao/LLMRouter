    ...
    def _load_router(self):
        ...
        try:
            ...
            if self.router_name == "randomrouter":
                from safety_routers.randomrouter.router import RandomRouter
                self.router = RandomRouter(self.config_path)

            elif self.router_name == "thresholdrouter":
                from safety_routers.thresholdrouter.router import ThresholdRouter
                self.router = ThresholdRouter(self.config_path)

            else:
                # Dynamic loading
                import importlib
                module = importlib.import_module(f"safety_routers.{self.router_name}.router")
