#!/usr/bin/env python3
"""
scripts/generate_queries_resumable.py - Resumable query generation

Usage:
    # Generate or continue (auto-detects existing file)
    python scripts/generate_queries_resumable.py --language vi --model minimax --output artifacts/safety_queries/queries_minimax_vni.jsonl

    # Force restart (will overwrite existing file after backup)
    python scripts/generate_queries_resumable.py --language vi --model minimax --output artifacts/safety_queries/queries_minimax_vni.jsonl --force-restart

The script parses existing file to determine:
- How many queries already exist
- Which query_id is the last one
Then continues generation from that point with the same seed.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Resumable safety query generation")
    ...
    if args.force_restart and stats["total"] > 0:
        ...
        stats = {"total": 0, "by_group_type": {}, "by_complexity": {}, "last_query_id": None, "query_ids": set()}
    
    # Calculate section offsets based on existing counts
    ...
    single_policy_batches = len(policies) * 3  # 12 * 3 = 36
    multi_policy_batches = args.multi_groups * 2  # 15 * 2 = 30
    no_policy_batches = 3
    ...
    start_query_id = f"Q{(total_queries + 1):04d}" if total_queries > 0 else "Q0001"
    ...
    print(f"\nGeneration will start from query_id: {start_query_id}")
    ...
    def streaming_callback(q):
        ...
        if total_count % 10 == 0:
            print(f"  Progress: {total_count} new queries written (query_id: {q.query_id})...")
    
    try:
        generation_offset = single_policy_count + multi_policy_count  # Skip all single + multi (completed sections)
